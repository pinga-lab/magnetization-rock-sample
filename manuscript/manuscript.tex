...
at an observation point located on the observation plane $\alpha$, 
the $z^{\prime}$-component of the magnetic induction (flux density) 
...
of the sensor.
The measured $z^{\prime}$-component at the point 
$(x^{\prime}, y^{\prime}, z^{\prime})$ on the plane
$\alpha$ (Figure \ref{fig:sample-planes-cross}) can be described as a 
two-dimensional convolution given by
\begin{equation}
\tilde{d}^{\prime}(x^{\prime}, y^{\prime}, z^{\prime}) = 
\int \limits_{-\infty}^{\infty} \int \limits_{-\infty}^{\infty}
w(\eta, \zeta) \; d^{\prime}(x^{\prime} - \eta, 
                             y^{\prime} - \zeta,
                             z^{\prime}) \, d\eta d\zeta \: ,
\label{eq:b-mag-flux}
\end{equation}
...
\begin{equation}
w(x^{\prime}, y^{\prime}) = \begin{cases}
\begin{split}
1/A &\: , \quad (x^{\prime}, y^{\prime}) \in A \\
0 &\: , \quad (x^{\prime}, y^{\prime}) \notin A
\end{split}
\end{cases} \: .
\label{eq:w-function}
\end{equation}
...
\begin{equation}
\tilde{d}^{\prime}(x^{\prime}, y^{\prime}, z^{\prime}) = \frac{1}{A} 
\int \limits_{-\infty}^{\infty} \int \limits_{-\infty}^{\infty}
d^{\prime}(x^{\prime} - \eta, y^{\prime} - \zeta, z^{\prime}) \, d\eta d\zeta \: .
...
$\Delta A$ (Figure \ref{fig:b-mag-flux-approx}b).
The approximated integral is given by
\begin{equation}
\tilde{d}^{\prime}_{i} \approx \frac{\Delta A}{A} \sum \limits_{j = 1}^{Q} d^{\prime}_{j} \: ,
\label{eq:b-mag-flux-approx}
\end{equation}
where $\tilde{d}^{\prime}_{i}$ is the measured 
$z^{\prime}$-component at the $i$th observation point
located at the center of the sensor active area $A$
(black circle in Figure \ref{fig:b-mag-flux-approx}b) and 
$d^{\prime}_{j}$ is the $z^{\prime}$-component at the
point $(x^{\prime}_{j}, y^{\prime}_{j})$, $j = 1, ..., Q$
(open circles in Figure \ref{fig:b-mag-flux-approx}b).
The $\tilde{d}^{\prime}_{i}$ measured on plane $\alpha$
is the $i$th element of the vector $\mathbf{d}^{0}_{z^{\prime}}$
(equation \ref{eq:obs-data-vec-LCS}).
Hence, by substituting $\Delta A = A/Q$ in equation {eq:b-mag-flux-approx}, 
we get
\begin{equation}
\tilde{d}^{\prime}_{i} \approx \frac{1}{Q} \sum \limits_{j = 1}^{Q} d^{\prime}_{j} \: .
\label{eq:b-mag-flux-approx2}
\end{equation}
Equation \ref{eq:b-mag-flux-approx2} shows that the measured
$z^{\prime}$-component $\tilde{d}^{\prime}_{i}$ 
at the $i$th observation point on a plane $\alpha$ is
...
%
 ...
 $(x^{\prime}, y^{\prime})$ (black circle). The square active area is discretized into 
 a regular grid of $Q$ square cells (limited by the dashed lines). Each
 cell is centered at a point $(x^{\prime}_{j}, y^{\prime}_{j})$, 
 $j = 1, ..., Q$, (open circles) and has an area $\Delta A$ equal to $A/Q$.}
