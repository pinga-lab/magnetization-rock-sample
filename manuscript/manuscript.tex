...
%This modification consists in calculating, on each observation plane
%$\alpha$ (Figures \ref{fig:sample-planes} and \ref{fig:sample-planes-cross}), 
%the theoretical $\beta$-component of the magnetic induction 
%produced by each prism forming the interpretation model
%not only at the observation points
%$(x_{i}^{\alpha}, y_{i}^{\alpha}, z_{i}^{\alpha})$, $i = 1, ..., N$, 
%$\alpha = 0, 1, 2, 3$, but at the regular grid 
%of $Q$ points (open circles in Figure \ref{fig:b-mag-flux-approx}b) centered
%at the $i$th observation point $(x_{i}^{\alpha}, y_{i}^{\alpha}, z_{i}^{\alpha})$
%(black circle in Figure \ref{fig:b-mag-flux-approx}b).
%This procedure increases the number of elements of the
%vectors $\mathbf{b}_{\beta}(\mathbf{x}^{\alpha}, \mathbf{y}^{\alpha},
%\mathbf{z}^{\alpha}, \mathbf{m}^{k})$ and matrices 
%$\mathbf{M}_{\beta}^{k}(\mathbf{x}^{\alpha}, \mathbf{y}^{\alpha},
%\mathbf{z}^{\alpha})$ (equation \ref{eq:pred-data-vec-k}),
%so that they become $QN \times 1$ and $QN \times 3$,
%respectively. Similarly, the vectors
%$\mathbf{B}_{\beta}^{\alpha}(\mathbf{m})$ 
%(equation \ref{eq:pred-data-vec-alphabeta})
%become $QN \times 1$.
%Consequently, the theoretical-predicted data vector 
%$\mathbf{B}(\mathbf{m})$ (equation \ref{eq:theor-pred-data-vec-matrix})
%become $4QN \times 1$ and the matrix 
%$\mathbf{M}$ (equation \ref{eq:theor-pred-data-vec-matrix}) 
%become a $4QN \times 3P$ matrix $\tilde{\mathbf{M}}$, respectively.
%Additionally, consider that the first $Q$ elements
%of each new vectors $\tilde{\mathbf{B}}_{\beta}^{\alpha}(\mathbf{m})$
%represent the $\beta$-component of the magnetic induction
%produced by the interpretation model at the regular grid of 
%$Q$ points centered at the observation point 
%$(x_{1}^{\alpha}, y_{1}^{\alpha}, z_{1}^{\alpha})$,
%the next $Q$ elements represent the $\beta$-component
%at the regular grid of 
%$Q$ points centered at the observation point 
%$(x_{2}^{\alpha}, y_{2}^{\alpha}, z_{2}^{\alpha})$ and
%so on.
...
the coordinates of the regular grid of $Q$ points centered
...
The $\beta$-component, $\beta = y, z$,
of the magnetic induction produced by the $k$-prism (in $nT$), at 
the observation points defined by the $N \times 1$ vectors 
$\mathbf{x}^{\alpha}$, $\mathbf{y}^{\alpha}$ and $\mathbf{z}^{\alpha}$,
$\alpha = 0, 1, 2, 3$,
...
\end{equation}
where $\mathbf{A}$ is a $QN \times QN$ matrix whose element $ij$ is given by
\begin{equation}
\left[\mathbf{A}\right]_{ij} = 
\begin{cases}
\begin{split}
1 &, \quad i \le j \le i+Q \\
...
$j = 1, ..., QN$, 
are second derivatives, evaluated at the points defined by
the $QN \times 1$ vectors 
$\tilde{\mathbf{x}}^{\alpha}$, $\tilde{\mathbf{y}}^{\alpha}$ and 
$\tilde{\mathbf{z}}^{\alpha}$,
of the function
\begin{equation}
...
magnetic constant and the integral is evaluated over the 
...
vectors $\mathbf{B}_{\beta}^{\alpha}(\mathbf{m})$ containing the theoretical 
...
conveniently rewrite the theoretical-predicted data vector 
...
and the matrix $\mathbf{A}$ defined in equation \ref{eq:Aij}.
