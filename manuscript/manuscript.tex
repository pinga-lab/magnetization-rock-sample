...
by the finite size of sensors. We limit our study to squared-planar 
sensors which are parallel to the sample surface.
Let us first consider the magnetic induction component $\tilde{b}(x,y)$ that is 
parallel to the sample surface and is threaded
through a planar sensing area parallel to the sample
as the following two-dimensional convolution
\begin{equation}
\tilde{b}(x, y) = \int \limits_{-\infty}^{\infty} \int \limits_{-\infty}^{\infty}
w(\eta, \zeta) \; b(x - \eta, y - \zeta) \, d\eta d\zeta \: ,
\label{eq:b-mag-flux}
\end{equation}
...
For convenience, we consider a square sensor having an active area $A$
(light grey square in Figure \ref{fig:b-mag-flux-approx}a).
In this case, the sensor answer function $w(\eta, \zeta)$ 
...
\label{eq:w-function}
\end{equation}
By substituting this sensor answer function into the
magnetic flux integral (equation \ref{eq:b-mag-flux})
we obtain
\begin{equation}
\tilde{b}(x, y) = \frac{1}{A} 
\int \limits_{-\infty}^{\infty} \int \limits_{-\infty}^{\infty}
b(x - \eta, y - \zeta) \, d\eta d\zeta \: .
...
a regular grid of $Q_{x} \times Q_{y}$ points $(x_{j}, y_{j}) \in A$,
$j = 1, ..., n$, where $Q = Q_{x} \, Q_{y}$.
The approximated integral is given by
\begin{equation}
\tilde{b}_{i} \approx \frac{\Delta A}{A} \sum \limits_{j = 1}^{Q} b_{j} \: ,
\label{eq:b-mag-flux-approx}
\end{equation}
where $\tilde{b}_{i} \equiv \tilde{b}(x_{i}, y_{i})$, $(x_{i}, y_{i})$ is
the point located at the center of the sensor active area $A$
(black circle in Figure \ref{fig:b-mag-flux-approx}b) and 
$b_{j}$ is the magnetic induction component parallel to the
sample surface and evaluated at the point $(x_{j}, y_{j})$
(open circle in Figure \ref{fig:b-mag-flux-approx}c),
$j = 1, ..., Q$.
According to the discretization represented in Figure 
\ref{fig:b-mag-flux-approx}b, the active sensor area is
divided into a regular grid of square cells with area
$\Delta A$ (Figure \ref{fig:b-mag-flux-approx}c). 
Hence, by noting that $A = Q \, \Delta A$, we get
\begin{equation}
\tilde{b}_{i} \approx \frac{1}{Q} \sum \limits_{j = 1}^{Q} b_{j} \: .
\label{eq:b-mag-flux-approx2}
\end{equation}
Equation \ref{eq:b-mag-flux-approx2} shows that the magnetic induction
$\tilde{b}_{i}$ at the observation point $(x_{i}, y_{i})$ is 
approximately given by the magnetic induction
averaged over the active area $A$ of the sensor.
...
%
 ...
 \caption{Schematic representation of the integral \ref{eq:Phi-mag-flux2}.
 (a) Magnetic sensor with active area $A$ (light grey square) located at
 a distance $h$ of the sample (grey prism). (b) Detailed representation
 of the sensor active area, which is centered at the observation point
 $(x_{i}, y_{i})$ (black circle). The square active area is discretized into 
 a regular grid of $Q$ square cells (limited by the dashed lines). 
 (c) jth cell with area $\Delta A$ and center at the point $(x_{j}, y_{j})$
 (open circle), $j = 1, ..., Q$.}
